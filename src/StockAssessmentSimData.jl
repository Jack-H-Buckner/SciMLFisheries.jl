...
function SurplusProduction(;seed = 123,T = 30, p = (r = 0.4, K = 10.0), F = up_down, Fmax = 0.25, q = 1.0,sigma_proc = 0.05, plot = true)
    ...
    B = p.K; Bls = []; Hls = []
    for t in 1:T
        B += p.r*B*(1-B/p.K) - F(t,T,Fmax)*B + B*rand(Normal(0,sigma_proc))
...
function SurplusProduction_forecast(;seed = 123,T = 30, p = (r = 0.4, K = 10.0), F = up_down, 
    ...
    for H in Hvals
        ...
        for t in 1:Tforecast
            B += p.r*B*(1-B/p.K) - H + B*rand(Normal(0,sigma_proc))
...
function simulation(derivs,noise,tspan,u0,tsteps,p,sigma_x,sigma_H)
    ...
    B0 = index[end]
    index = 4.0 * index ./ B0
    y = log.(index) .+ rand(Normal(0.0,sigma_x), length(ode_data[1,2:end]))
        
    H = 4.0 * (ode_data[2,2:end] .- ode_data[2,1:(end-1)])./B0 .+ rand(Normal(0.0,sigma_H), length(ode_data[2,2:end]))
...
function simulate_test_data(derivs,noise,u0,p,testing_tspan,testing_tsteps,Hvals,sigma_x,B0)
    ...
    for H in Hvals
        ...
        index = 4.0 *index ./ B0
        y = log.(index) .+ rand(Normal(0.0,sigma_x), length(ode_test_data[1,2:end]))
...
function AgeStuructured(;plot = true, T = 30,
    ...
    y = log.(index) .+ rand(Normal(0.0,sigma_x), length(T))
        
    H = 4.0 * Hls./B0 .+ Hls.*rand(Normal(0.0,sigma_H), length(T))
