...
function Logistic_LSTM_process(hidden,cell_dim;steps = 10, seed = 123)
    ...
    parameters = (log_r = log(0.05), log_K = log(3),log_q = log(0.1), sigma = 0.5,Dense = dense_parameters, LSTM = LSTM_parameters, x0 = [0.0])
    ...
    function predict(u,dt,parameters)
        ...
        x = u[1]
        for i in 1:steps
            rt = r[1] + exp(parameters.log_r).*(1 - exp(x)/exp(parameters.log_K))
            x +=  dt*rt/steps - dt*exp(parameters.log_q)*u[2]/exp(x)/steps
        end
...
function init_loss_LSTM(times,data,predict,ratio, mu_log_q, sigma_log_q, mu_log_K,sigma_log_K, mu_log_r, sigma_log_r, sigma_reg)
...
mutable struct spLogisticLSTM_
    data
    times
    predict
    predict_F
...
function spLogisticLSTM(data, time_column_name, harvest_column_name, index_column_name;
                            hidden = 5,cell_dim=2,ratio = 2.0, k = 2, phi = 0.0025,
                            mu_log_q = 0.0,sigma_log_q = 100.0,
                            mu_log_r = 0.0,sigma_log_r = 100.0,
                            mu_log_K = 0.0,sigma_log_K = 100.0,
                            sigma_reg = 0.5,step_size = 0.0025, maxiter = 500,
                            steps = 10)
    ...
    predict, predict_F, parameters =Logistic_LSTM_process(cell_dim,hidden;steps = steps)
    priors = (k=k, phi=phi, mu_log_q=mu_log_q, sigma_log_q=sigma_log_q,sigma_reg=sigma_reg) 
    loss =  init_loss_LSTM(times,data, predict,ratio,  mu_log_q, sigma_log_q, mu_log_K, sigma_log_K, mu_log_r, sigma_log_r, sigma_reg)
    ...
    return spLogisticLSTM_(dataframe, times,predict_sol, predict_F_sol, sol.u, priors, time_column_name, index_column_name, harvest_column_name)
...
function forecast(model::spLogisticLSTM_,new_data,fishing_column_name,fishing_variable)
